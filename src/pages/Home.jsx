          ...
          <div ref={trackRef} className="cats-scroll-track" style={{ transform: `translateX(-${offset}px)` }}>
            {categories.map((cat) => (
              <div key={cat.id} className="cat-big-card" onClick={() => navigate(`/categorie/${cat.slug}`)}>
                {cat.image ? (
                  <img src={`https://mblifestyle-backend-production.up.railway.app/storage/${cat.image}`} alt={cat.name} className="cat-big-img" />
                ) : (
    ...
    <section className="new-arrivals-section">
      <div className="new-arrivals-inner">
        <div className="new-arrivals-featured">
          {featuredImage ? (
            <img src={`https://mblifestyle-backend-production.up.railway.app/storage/${featuredImage}`} alt="Nouveautés" className="new-arrivals-featured-img" />
          ) : (
                ...
                {slide.style === 'fagor-banner' ? (
                  <div className="slide-fagor-banner">
                    ...
                    <div className="sfb-photo">
                      {slide.product_image && <img src={`https://mblifestyle-backend-production.up.railway.app/storage/${slide.product_image}`} alt={slide.title} />}
                    </div>
                    <div className="sfb-badge"><span>QUALITÉ<br/>GARANTIE</span><div className="sfb-badge-stars">★★★★★</div></div>
                  </div>
                ) : slide.video ? (
                  <video className="slide-video" autoPlay muted loop playsInline src={`https://mblifestyle-backend-production.up.railway.app/storage/${slide.video}`} />
                ) : (
                  <div className="slide-bg" style={{
                    backgroundImage: slide.image ? `url(https://mblifestyle-backend-production.up.railway.app/storage/${slide.image})` : 'none',
            ...
            <div className={`lookbook-grid lookbook-grid-${lookbook.length}`}>
              {lookbook.map((item, i) => (
                <div key={i} className={`lookbook-card scroll-animate-child ${lookbookVisible ? 'scroll-visible' : ''}`}
                  ...
                  <img src={`https://mblifestyle-backend-production.up.railway.app/storage/${item.image}`} alt={item.title} className="lookbook-img" />
              ...
              {popup.products.map((product, i) => (
                <div key={product.id} className={`flash-card scroll-animate-child ${flashVisible ? 'scroll-visible' : ''}`}
                  ...
                  <div className="flash-card-img">
                    {product.images?.[0] ? <img src={`https://mblifestyle-backend-production.up.railway.app/storage/${product.images[0]}`} alt={product.name} /> : <div className="flash-no-img">👕</div>}
      ...
      <div ref={promoRef} className={`fagor-banner scroll-animate ${promoVisible ? 'scroll-visible' : ''}`}>
        <div className="fagor-banner__photo">
          <div className="fagor-banner__circle">
            {promoData.image ? (
              <img src={`https://mblifestyle-backend-production.up.railway.app/storage/${promoData.image}`} alt="Promo" />
            ) : (
